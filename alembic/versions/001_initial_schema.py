...
def upgrade() -> None:
    # Create enums
    op.execute("CREATE TYPE prompt_version_status AS ENUM ('draft', 'active', 'deprecated')")
    op.execute("CREATE TYPE execution_mode AS ENUM ('sync', 'async')")
    op.execute("CREATE TYPE execution_status AS ENUM ('queued', 'running', 'succeeded', 'failed', 'canceled')")
    op.execute("CREATE TYPE provider_name AS ENUM ('openai')")

    # Create prompts table
    op.create_table('prompts',
        sa.Column('prompt_id', postgresql.UUID(as_uuid=True), nullable=False, default=sa.text('gen_random_uuid()')),
    ...
    op.create_table('prompt_versions',
        sa.Column('version_id', postgresql.UUID(as_uuid=True), nullable=False, default=sa.text('gen_random_uuid()')),
        sa.Column('prompt_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('version_number', sa.Integer(), nullable=False),
        sa.Column('status', sa.Enum('draft', 'active', 'deprecated', name='prompt_version_status'), nullable=False),
    ...
    op.create_table('models',
        sa.Column('model_id', postgresql.UUID(as_uuid=True), nullable=False, default=sa.text('gen_random_uuid()')),
        sa.Column('provider', sa.Enum('openai', name='provider_name'), nullable=False),
    ...
    op.create_table('executions',
        ...
        sa.Column('execution_mode', sa.Enum('sync', 'async', name='execution_mode'), nullable=False),
        sa.Column('status', sa.Enum('queued', 'running', 'succeeded', 'failed', 'canceled', name='execution_status'), nullable=False, default='queued'),
    ...
    # Create indexes
    op.create_index('idx_exec_prompt_time', 'executions', ['prompt_id', sa.text('created_at DESC')])
    op.create_index('idx_exec_version_time', 'executions', ['version_id', sa.text('created_at DESC')])
    op.create_index('idx_exec_status_time', 'executions', ['status', sa.text('created_at DESC')])
    op.create_index('idx_exec_corr', 'executions', ['correlation_id'])
    op.create_index('uq_exec_idempotency', 'executions', ['prompt_id', 'idempotency_key'], unique=True, postgresql_where=sa.text('idempotency_key IS NOT NULL'))

    # Add foreign key constraint for active_version_id
    op.create_foreign_key('fk_prompts_active_version', 'prompts', 'prompt_versions', ['active_version_id'], ['version_id'])
