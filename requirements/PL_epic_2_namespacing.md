...
`GET /v1/admin/projects` — list all projects (admin key required):
```json
[
  { "project_id": "<uuid>", "name": "default", "created_at": "..." },
  { "project_id": "<uuid>", "name": "srf",     "created_at": "..." }
]
```

`GET /v1/admin/projects/{project_id}/keys` — list key metadata for a project. **Never returns
plaintext or hashes** — only enough to identify which key to act on:
```json
[
  { "key_id": "<uuid>", "label": "srf-production", "created_at": "...", "is_system_key": false },
  { "key_id": "<uuid>", "label": "srf-staging",    "created_at": "...", "is_system_key": false }
]
```
This is the recovery path when a team loses their key: admin lists keys, issues a new one,
team updates their env var, admin revokes the old key — all without touching the database.
...
(also the recovery path when a key is lost):
```json
// Request
{ "label": "srf-production-replacement" }
...
**Key loss recovery procedure** (zero-downtime):
1. `GET /v1/admin/projects` → find the project's `project_id`
2. `POST /v1/admin/projects/{project_id}/keys` → issue replacement key
3. Team updates `PROMPTLEDGER_API_KEY` in their deployment
4. `GET /v1/admin/projects/{project_id}/keys` → identify the lost key's `key_id` by label/date
5. `DELETE /v1/admin/keys/{key_id}` → revoke the lost key

Steps 2–3 can overlap: the old key (if known to the attacker) and new key are both valid
during the transition window. Revoke the old key only after the new key is confirmed working.
...
- `DELETE /v1/admin/keys/{key_id}` must refuse to delete system keys (keys with
  `is_system_key = true`) — deleting the seeded env-var key would lock out the default
  project entirely. Return 409 Conflict with a clear error message.

**Schema addition to `api_keys`:**
```sql
ALTER TABLE api_keys ADD COLUMN is_system_key BOOLEAN NOT NULL DEFAULT false;
```
Set `is_system_key = true` when seeding the env-var key at startup. System keys can only
be rotated by updating `API_KEY` in the environment and restarting (which re-seeds).

**Tests (write first):**
- `GET /v1/admin/projects` with a non-default API key returns 403
- `GET /v1/admin/projects` returns all projects; `name` and `project_id` present, no key material
- `GET /v1/admin/projects/{project_id}/keys` returns key metadata; response contains no
  plaintext key or hash — only `key_id`, `label`, `created_at`, `is_system_key`
- `GET /v1/admin/projects/{project_id}/keys` for an unknown project returns 404
...
- `POST /v1/admin/projects/{project_id}/keys` (key loss recovery) issues a new valid key for
  the project; old key continues to work until explicitly revoked
- `DELETE /v1/admin/keys/{key_id}` causes subsequent requests with that key to return 401
- `DELETE /v1/admin/keys/{key_id}` on the seeded system key returns 409 Conflict
- Attempting to retrieve the plaintext key after creation is not possible

---

### Story 2.5 — Documentation Update

**User Story:**
> As a **developer evaluating or operating PromptLedger**, I want up-to-date documentation
> covering multi-tenant deployment, project creation, API key management, and SDK usage in
> a namespaced environment, so that I can onboard new projects without reading source code
> or reverse-engineering the admin API.

**Goal:** All public-facing docs reflect the multi-tenant model introduced in Epic 2. A new
operator deploying PromptLedger on Railway should be able to create a project, issue a key,
and have a consuming application (e.g. SRF) authenticated and scoped correctly by following
the docs alone.

**Files to update:**

`README.md`:
- Add "Multi-tenant / project namespacing" to the features list
- Update the authentication section to describe DB-backed keys (not just `API_KEY` env var)
- Add a "Creating a project" quick-start snippet using `POST /v1/admin/projects`

`INTEGRATION_GUIDE.md`:
- New section: **Project Setup** — before any integration steps, explain that the consuming
  project needs its own API key. Show the `POST /v1/admin/projects` call and how to store
  the returned key as `PROMPTLEDGER_API_KEY` in the consuming project's env.
- Update the SDK initialisation example to reflect that the `api_key` passed to
  `AsyncPromptLedgerClient` is now a project-scoped key, not the operator's root key.
- Add a note on key rotation and key loss recovery: issue a new key first, update the
  consuming app, then revoke the old key — zero-downtime in both cases. Make explicit that
  plaintext keys are unrecoverable by design; the recovery path is always "issue new key."
- Update the CI/CD dry-run section: the key used in CI should be the project-scoped key,
  not the admin key.

`ARCHITECTURE.md`:
- Update the auth section to describe the DB-backed key lookup, TTL cache, and
  `project_id` propagation through request state.
- Add a `projects` / `api_keys` entry to the data model diagram.

`.env.example`:
- Add a comment explaining that `API_KEY` is the **admin / default project key** — it
  should not be used directly in consuming applications. Consuming projects get their own
  keys via `POST /v1/admin/projects`.

`client/README.md` (SDK docs):
- Confirm that `AsyncPromptLedgerClient(base_url, api_key)` expects a **project-scoped**
  key, not the admin key.
- Add a one-liner linking to the operator docs for how to create a project and get a key.

**Acceptance:** A developer who has never seen the PromptLedger codebase can:
1. Deploy PromptLedger on Railway
2. Create a project for their application using only the docs
3. Integrate `promptledger-client` with the issued key
4. Confirm their spans and prompts are isolated from other projects

**Tests:** Documentation-only story — no automated tests. Acceptance is manual review
...
2.1  Projects + API keys model   (new tables, seeding, auth middleware upgrade)
     │
     ├── 2.2  Prompt scoping     (migration 2, PromptService query changes)
     │
     ├── 2.3  Span + execution   (migration 3, ExecutionService + analytics scoping)
     │        scoping
     │
     └── 2.4  Admin API          (new admin.py router, no schema changes)
              │
              └── 2.5  Documentation   (README, INTEGRATION_GUIDE, ARCHITECTURE,
                                        .env.example, client/README)
```

**2.1 is the only hard prerequisite** — it creates the `projects`/`api_keys` tables and
upgrades auth to return `project_id`. Everything else unblocks after it.

**2.2, 2.3, and 2.4 can run in parallel** — they touch entirely separate tables, services,
and endpoint files:
- 2.2 → `prompts` table, `PromptService`, prompts/code_prompts endpoints
- 2.3 → `spans`/`executions` tables, `ExecutionService`, spans/analytics endpoints
- 2.4 → new `admin.py` router only, no changes to existing files

**2.5** can be drafted in parallel with 2.2–2.4 once the API surface is stable after 2.1.
It needs a final review pass after the others land but should not block the release.

**Critical path: ~4.5 days** (2.1 → longest parallel branch → 2.5 final pass),
vs ~7–9 days if run sequentially. With two contributors, 2.2+2.4 and 2.3 can be split.
...
# Story 2.4 — list projects and keys (no key material in response)
curl -H "X-API-Key: $LEGACY_KEY" $PL_URL/v1/admin/projects
# Expect: JSON array with project_id and name for each project

PROJECT_ID=$(curl -H "X-API-Key: $LEGACY_KEY" $PL_URL/v1/admin/projects | jq -r '.[] | select(.name=="srf") | .project_id')
curl -H "X-API-Key: $LEGACY_KEY" $PL_URL/v1/admin/projects/$PROJECT_ID/keys
# Expect: array of key metadata (key_id, label, created_at, is_system_key) — no plaintext

# Story 2.4 — key loss recovery (zero-downtime)
NEW_KEY=$(curl -X POST $PL_URL/v1/admin/projects/$PROJECT_ID/keys \
  -H "X-API-Key: $LEGACY_KEY" -d '{"label":"srf-production-replacement"}' | jq -r .api_key)
curl -H "X-API-Key: $NEW_KEY" $PL_URL/v1/prompts
# Expect: 200 (new key works before old key is revoked)

# Story 2.4 — revoke old key
curl -X DELETE $PL_URL/v1/admin/keys/$OLD_KEY_ID -H "X-API-Key: $LEGACY_KEY"
curl -H "X-API-Key: $REVOKED_KEY" $PL_URL/v1/prompts
# Expect: 401

# Story 2.4 — system key protection
curl -X DELETE $PL_URL/v1/admin/keys/$SYSTEM_KEY_ID -H "X-API-Key: $LEGACY_KEY"
# Expect: 409 Conflict

# Story 2.5 — docs review
# Manual: follow README "Creating a project" section end-to-end on a fresh Railway deploy
# Manual: confirm INTEGRATION_GUIDE project setup section produces a working integration
```
